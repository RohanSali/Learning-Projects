...
import numpy as np
from .snake import Grid, Snake, Apple, Poop

# Simple Snake with Postion-dependent environment
class SimpleSnake(BaseEnvironment):
    """Environment for Snake with state space of len 13 and action space of len 3"""
    ...
    # Actions in our env comprise of : ahead, right, left
    ...
    def env_step(self, action):
        ...
        reward, terminal = self.get_reward(eaten=eaten, is_grid_full=observation[-1])
    ...
    def get_reward(self, eaten, is_grid_full):
        reward = 0.0
        terminal = False
        ...
        reward -= action_penulty    # Penulty for each action
...
# Simple Snake with  ostion-independent environment    
class SimpleSnake2(SimpleSnake):
    """Environment for Snake (Position Independent) with state space of len 11 and action space of len 3"""
...
# Snake with Wink action and Postion-dependent environment
...
# Snake with Wink action and Postion-independent environment    
...
# Snake with Wink action, Cell-Blockage through Poop and Postion-dependent environment
class WinkNPoopSnake(BaseEnvironment):
    """Environment for Snake with state space of len 17 and action space of len 4"""
    # State in our env comprise of : snake_head, snake_facing, snake_length, closest_food_distance, food_intuition_vector, danger_vector, poop_info_vector, poop_count, is_grid_full
    # 1. snake_head : Position of head of snake in grid as [cell_x, cell_y]
    # 2. snake_facing : Current facing of snake in grid as 0: Up, 1: Right, 2: Down, 3: Left
    # 3. snake_length : Length of snake as integer
    # 4. closest_food_distance : Manhatton distance from Closest apple in grid as [cell_x, cell_y]
    # 5. food_intuition_vector : Tells if in that direction food is present or not. as [Up, Right, Down, Left] ; if [1,1,0,0] tells food in top, right side. (Relative to Grid Directions)
    # 6. danger_vector : If danger in front or adjacent cells where it is facing then it is 1 otherwise 0. as [Ahead, Right, Left] ; if [1,1,0] tells wall or body in top and right side. (Relative to Snake Facing)
    # 7. poop_info_vector : If poop present in front or adjacent cells where it is facing then it is 1 otherwise 0. as [Ahead, Right, Left] ; if [1,1,0] tells poops are in top cell and right side cell. (Relative to Snake Facing)
    # 8. poop_count : Repredents total number of poops; Poops are the consequence of wink action.
    # 9. is_grid_full : It is 1 if grid is full and 0 otherwise.
    # Finally a state is : ndarray as : [head_x, head_y, head_facing, snake_length, closest_food_distance, 
    #                                    is_food_up, is_food_right, is_food_down, is_food_left, 
    #                                    is_danger_ahead, is_danger_right_side, is_danger_left_side, 
    #                                    is_poop_ahead, is_poop_right_side, is_poop_left_side, 
    #                                    poop_count, is_grid_full]
    ...
    def __init__(self):
        super().__init__()
        self.current_state = np.zeros(17)
        self._intialized = False
        self.nodes_added = 0
        self.no_food_eaten_count = 0
    ...
    def env_step(self, action):
        eaten, poop_cleared = self.perform_action(action)

        observation = self.get_observation()
        self.current_state = observation

        reward, terminal = self.get_reward(action=action, eaten=eaten, poop_cleared=poop_cleared, is_grid_full=observation[-1])
    ...
    def env_message(self, message):
        ...
        if message == "debug":
            return {
                ...
                "length": self.snake.length,
                "apples": [a.position for a in self.apples],
                "poops": [p.position for p in self.poops],
            }
    ...
    def get_observation(self):
        ...
        poop_positions = [poop.position for poop in self.poops]
        poop_info_vector = get_poop_info_vector(snake_head, snake_facing, poop_positions)
        poop_count = len(self.poops)

        is_grid_full = 1 if (self.apple_count + self.snake.length + len(self.poops) >= (self.grid.layout[0] * self.grid.layout[1])) else 0
    ...
    def perform_action(self, action):
        ...
        if action == 0:
            ...
            if self.snake.winked :
                pos = self.snake.tail.last_pos
                poop = Poop(position=pos)
                self.poops.append(poop)

            self.snake.winked = False
        ...
        for apple in list(self.apples):
            if apple.eaten:
                ...
                a_cell_x, a_cell_y = random.randrange(self.grid.layout[0]), random.randrange(self.grid.layout[1])
                while [a_cell_x, a_cell_y] in  apple_positions + snake_positions + poop_positions:
                    a_cell_x, a_cell_y = random.randrange(self.grid.layout[0]), random.randrange(self.grid.layout[1])

                new_apple = Apple(position=[a_cell_x, a_cell_y])
                self.apples.append(new_apple)
        
        for poop in list(self.poops):
            if self.snake.head.position == poop.position:
                poop_cleared = True
                self.poops.remove(poop)
                break

        return is_eaten, poop_cleared
    
    def get_reward(self, action, eaten, poop_cleared, is_grid_full):
        reward = 0.0
        terminal = False
        fruit_reward = 5  # or 10
        action_penulty = 0.02  # or 0.01
        death_penulty = 20  # or 50
        poop_clear_penulty = 10 # or 30
        k = 5
...
def get_poop_info_vector(snake_head, snake_facing, poop_locations):
    if len(poop_locations) == 0:
        return [0, 0, 0]
    ...
    def is_poop(x, y):
        return 1 if [x, y] in poop_locations else 0
